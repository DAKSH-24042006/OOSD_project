...
public class CSVHandler {
    ...
    public static void ensureFilesExist() {
        createFileIfNotExists(ROOMS_FILE, "roomId,type,price,status,floor,bedType,ac,wifi,cleaningStatus");
            ...
            try (BufferedReader br = new BufferedReader(new FileReader(ROOMS_FILE))) {
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    try {
                        ...
                        if (data.length >= 8) {
                            String cleaningStatus = data.length >= 9 ? data[8].trim() : "CLEAN";
                            rooms.add(new Room(Integer.parseInt(data[0].trim()), data[1].trim(), Double.parseDouble(data[2].trim()), data[3].trim(),
                                    Integer.parseInt(data[4].trim()), data[5].trim(), data[6].trim(), data[7].trim(), cleaningStatus));
                        }
    ...
    public static void saveRooms(List<Room> rooms) {
        ...
        try {
            try (PrintWriter pw = new PrintWriter(new FileWriter(ROOMS_FILE))) {
                pw.println("roomId,type,price,status,floor,bedType,ac,wifi,cleaningStatus");
                for (Room room : rooms) {
                    pw.printf("%d,%s,%s,%s,%d,%s,%s,%s,%s%n", room.getRoomId(), room.getType(), room.getPrice(),
                            room.getStatus(), room.getFloor(), room.getBedType(), room.getAc(), room.getWifi(), room.getCleaningStatus());
                }
            } catch (IOException ignored) {}
        } finally {
            roomsLock.unlock();
    ...
    public static void syncAllToDB() throws SQLException {
        ...
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement("INSERT INTO rooms (roomId, type, price, status, floor, bedType, ac, wifi, cleaning_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) " +
                     "ON DUPLICATE KEY UPDATE type=VALUES(type), price=VALUES(price), status=VALUES(status), floor=VALUES(floor), bedType=VALUES(bedType), ac=VALUES(ac), wifi=VALUES(wifi), cleaning_status=VALUES(cleaning_status)")) {
            for (Room r : rooms) {
                ...
                pstmt.setString(9, r.getCleaningStatus());
                pstmt.addBatch();
            }
            pstmt.executeBatch();
        }
        ...
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement("INSERT INTO customers (customerId, name, phone, email, address) VALUES (?, ?, ?, ?, ?) " +
        ...
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement("INSERT INTO bookings (bookingId, customerId, roomId, checkIn, checkOut, paymentStatus, paymentMethod, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?) " +
                     "ON DUPLICATE KEY UPDATE customerId=VALUES(customerId), roomId=VALUES(roomId), checkIn=VALUES(checkIn), checkOut=VALUES(checkOut), paymentStatus=VALUES(paymentStatus), paymentMethod=VALUES(paymentMethod), status=VALUES(status)")) {
        ...
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement("INSERT INTO billing (billId, bookingId, totalAmount, billDate, tax, discount, extraCharges) VALUES (?, ?, ?, ?, ?, ?, ?) " +
                     "ON DUPLICATE KEY UPDATE bookingId=VALUES(bookingId), totalAmount=VALUES(totalAmount), billDate=VALUES(billDate), tax=VALUES(tax), discount=VALUES(discount), extraCharges=VALUES(extraCharges)")) {
            for (Bill b : bills) {
