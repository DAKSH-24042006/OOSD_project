...
public class RoomController {
    ...
    @FXML private TableColumn<Room, String> colCleaningStatus;
    ...
    public void initialize() {
        ...
        colCleaningStatus.setCellFactory(column -> new TableCell<Room, String>() {
            @Override
            protected void updateItem(String item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) {
                    ...
                    Label label = new Label(item);
                    if ("CLEAN".equalsIgnoreCase(item)) {
                        label.setStyle("-fx-background-color:#1a472a;-fx-text-fill:#69db7c;-fx-background-radius:12;-fx-padding:3 10;-fx-font-weight:bold;");
                    } else if ("DIRTY".equalsIgnoreCase(item)) {
                        label.setStyle("-fx-background-color:#3b1a1a;-fx-text-fill:#ff6b6b;-fx-background-radius:12;-fx-padding:3 10;-fx-font-weight:bold;");
    ...
    private void handleAddRoom() {
        try {
            ...
            Room newRoom = new Room(roomId, type, price, "Available", floor, bedType, ac, wifi, "CLEAN");
    ...
    private void handleStartCleaning() {
        if (selectedRoom == null) return;
        if (!"DIRTY".equalsIgnoreCase(selectedRoom.getCleaningStatus())) {
            showAlert("Action Not Allowed", "Only dirty rooms can be cleaned.");
            return;
        }
        util.CleaningManager.assignCleaning(selectedRoom, () -> {
            tableRooms.refresh();
        });
        tableRooms.refresh();
    }
    ...
    private void saveRoomToDB(Room room) {
        String sql = "INSERT INTO rooms (roomId, type, price, status, floor, bedType, ac, wifi, cleaning_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        executeDBStatement(sql, room);
    }

    private void updateRoomInDB(Room room) {
        String sql = "UPDATE rooms SET type=?, price=?, status=?, floor=?, bedType=?, ac=?, wifi=?, cleaning_status=? WHERE roomId=?";
        try (Connection conn = DBConnection.getConnection();
            ...
            synchronized (DBConnection.getDbMonitor()) {
                ...
                pstmt.setString(8, room.getCleaningStatus());
    ...
    private void executeDBStatement(String sql, Room room) {
        try (Connection conn = DBConnection.getConnection();
            ...
            synchronized (DBConnection.getDbMonitor()) {
                ...
                pstmt.setString(9, room.getCleaningStatus());
                pstmt.executeUpdate();
            }
        } catch (SQLException e) {
            showDBError(e);
