...
/**
 * Controller for the new BillingProcess page.
 * Provides full checkout and billing functionality with high stability.
 */
public class BillingProcessController {
    ...
    @FXML public Label stayDurationLabel;
    ...
    public void initialize() {
        try {
            ...
            extraCostField.textProperty().addListener((obs, oldV, newV) -> updateCalculations());
            discountPercentField.textProperty().addListener((obs, oldV, newV) -> updateCalculations());
            
            System.out.println("DEBUG: BillingProcessController initialized successfully.");
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
    ...
    private void onBookingChosen(int bid) {
        ...
        if (activeSelection != null) {
            ...
            if (guestNameLabel != null) guestNameLabel.setText(c != null ? c.getName() : "Loading...");
            ...
            if (r != null) {
                ...
                if (days <= 0) days = 1;
                if (stayDurationLabel != null) stayDurationLabel.setText(days + " Days");
                calculatedBaseRate = r.getPrice() * days;
            } else {
                if (roomInfoLabel != null) roomInfoLabel.setText("Unknown Room");
                if (stayDurationLabel != null) stayDurationLabel.setText("-");
    ...
    private void updateCalculations() {
        try {
            double extra = 0;
            try { 
                if (extraCostField != null && !extraCostField.getText().isEmpty()) 
                    extra = Double.parseDouble(extraCostField.getText());
            } catch (Exception ignored) {}
            
            double discPercent = 0;
            try {
                if (discountPercentField != null && !discountPercentField.getText().isEmpty()) 
                    discPercent = Double.parseDouble(discountPercentField.getText());
            } catch (Exception ignored) {}
            
            double subtotal = calculatedBaseRate + extra;
    ...
    public void submitCheckout() {
        ...
        try {
            ...
            Bill bill = new Bill(CSVHandler.nextBillId(), activeSelection.getBookingId(), total, 
            ...
            if (room != null) {
                ...
                CSVHandler.saveRooms(rooms.stream()
                    .map(rm -> rm.getRoomId() == room.getRoomId() ? room : rm)
                    .collect(Collectors.toList()));
                updateDBRoomStatus(room.getRoomId(), "Available");
            }

            showMessage("Success", "Billing generated successfully.");
            resetUI();
            updateBookingOptions();
        } catch (Exception e) {
            showMessage("Error", "Billing failed: " + e.getMessage());
        }
    }

    private void writeBillToDB(Bill b) {
    ...
    private void updateDBRoomStatus(int roomId, String status) {
