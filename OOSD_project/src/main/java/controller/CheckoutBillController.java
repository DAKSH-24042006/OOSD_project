...
public class CheckoutBillController {
    ...
    public void onCompleteCheckout() {
        ...
        try {
            ...
            boolean dbSuccess = persistBillToDB(newBill);
            if (!dbSuccess) return;

            Room room = CSVHandler.getRoomById(selectedBooking.getRoomId());
            if (room != null) {
                boolean roomDbSuccess = syncRoomStatusToDB(room.getRoomId(), "Available", "DIRTY");
    ...
    private boolean persistBillToDB(Bill b) {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            javafx.application.Platform.runLater(() -> displayMessage("Database Error", "Could not save bill: " + e.getMessage()));
            return false;
        }
    }

    private boolean syncRoomStatusToDB(int roomId, String status, String cleaningStatus) {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            javafx.application.Platform.runLater(() -> displayMessage("Database Error", "Could not update room: " + e.getMessage()));
            return false;
        }
    }
