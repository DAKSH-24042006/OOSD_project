...
public class CustomerController {
    ...
    private void handleSave() {
        if (!validate()) {
            showAlert("Validation Error", "Please enter a valid name, email, and Indian phone number.");
            return;
        }
    ...
    private boolean validate() {
        if (txtName == null || txtPhone == null || txtEmail == null) return false;
        ...
        return !txtName.getText().isEmpty() && 
               phone.matches("^(\\+91|0)?[6-9]\\d{9}$") && 
               txtEmail.getText() != null && txtEmail.getText().contains("@");
    }
